...
//  Demo
//
//  Created by Begum Unal on 2.12.2023.
//
...
var keywordStrings = deneme.replacingOccurrences(of: "\n", with: " ")
...
private func start() {
  for calibrationWord in getCalibrationValueList() {
    for character in calibrationWord {
    ...
    var digitKey = "\(firstDigit)\(lastDigit)"
    totalNumber += Int(digitKey) ?? 0
    digitKey = ""
    counter = 0
  }
...
func getCalibrationValueList() -> [String] {
  var word = ""
  var startedIndex = 0
  var calibrationList = [String]()
  
  for (i, char) in keywordStrings.enumerated() {
    if char == " " {
      for j in startedIndex...i {
        let charAtIndex = keywordStrings[keywordStrings.index(keywordStrings.startIndex, offsetBy: j)]
