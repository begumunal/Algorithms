...
private class A {
  private var keyword = data.replacingOccurrences(of: "\n", with: " ")
  ...
  private let letters = Letter.allCases
  
  func findMaxIndex(_ array: [Int]) -> (maxValue: Int, maxIndex: Int)? {
    guard let maxElement = array.max() else { return nil }
    if let maxIndex = array.firstIndex(of: maxElement) {
      return (maxValue: maxElement, maxIndex: maxIndex)
    }
    
    return nil
  }

  func findMinIndex(_ array: [Int]) -> (minValue: Int, minIndex: Int)? {
    guard let minElement = array.min() else { return nil }
    if let minIndex = array.firstIndex(of: minElement) {
      return (minValue: minElement, minIndex: minIndex)
    }
    return nil
  }

  private func start() {
    var letterStartedIndexNumbers = [Int]()
    var numbers = [Int]()
    var totalNumber = 0
    
    for word in makeKeyword() {
      for letter in letters {
        var startIndex1 = word.startIndex
        var startIndex2 = word.startIndex
        
        while startIndex1 < word.endIndex, let range = word[startIndex1...].range(of: letter.stringValue, options: .literal) {
          let lowerBoundIndex = word.distance(from: word.startIndex, to: range.lowerBound)
          
          letterStartedIndexNumbers.append(lowerBoundIndex)
          numbers.append(letter.intValue)
          
          startIndex1 = range.upperBound
        }

        while startIndex2 < word.endIndex,
              let range = word[startIndex2...].range(of: letter.numberValue, options: .literal) {
          ...
          startIndex2 = range.upperBound
        }
      }
      
      let maxIndex = findMaxIndex(letterStartedIndexNumbers)?.maxIndex
      let minIndex = findMinIndex(letterStartedIndexNumbers)?.minIndex
      
      if let maxIndex = maxIndex, let minIndex = minIndex {
        let number = Int("\(numbers[minIndex])\(numbers[maxIndex])")
        totalNumber += number ?? 0
      }
      
      letterStartedIndexNumbers = []
      numbers = []
  ...
  private func makeKeyword() -> [String] {
    var word = ""
    var startedIndex = 0
    var calibrationList = [String]()
    
    for (i, char) in keyword.enumerated() {
      if char == " " {
        for j in startedIndex...i {
          let charAtIndex = keyword[keyword.index(keyword.startIndex, offsetBy: j)]
          word.append(charAtIndex)
        }
        
        calibrationList.append(word)
